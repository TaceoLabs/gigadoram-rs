...
use primitives::{Block, low_u32, random_indexed_block, random_indexed_blocks, run_parties};
...
fn test_lookup_missing_key() {
    ...
    let missing_key = loop {
        let left = rng.r#gen::<usize>() & mask;
        let right = rng.r#gen::<usize>() & mask;
        let entry = random_indexed_block(LOG_SINGLE_COL_LEN, left, right, 42);
        let key = entry & !0xffff_ffffu128;
        let tag = key >> 32;
...
fn setup() -> (Vec<Block>, Vec<Block>, Vec<usize>) {
    let input = random_indexed_blocks(LOG_SINGLE_COL_LEN, INPUT_COUNT);
