...
impl OhTable {
    ...
    fn shuffle_receiver_order<N: Network>(
        ...
        Ok(if state.id == self.params.builder.next() {
            receiver_shuffler.next_shared_perm
        } else {
            receiver_shuffler.prev_shared_perm
        })
    }
    ...
    fn place_stash<N: Network>(
        ...
        mut receiver_shuffle: LocalPermutation,
        net: &N,
        state: &Rep3State,
    ) -> eyre::Result<()> {
        let builder_stash_indices = match state.id {
            ...
            _ => net.recv_from(self.params.builder)?,
        };
    ...
    fn reveal_qs_to_builder<N: Network>(
        ...
        match state.id {
            id if id == self.params.builder => {
                let prev_b =
                    net.recv_from::<Vec<RingElement<Block>>>(self.params.builder.prev())?;
                Ok(Some(
