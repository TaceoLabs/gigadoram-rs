...
use mpc_net::Network;
use primitives::{
    ArrayShuffler, Block, BlockShare, LocalPermutation, XShare, YRecord, alibi_from_blocks,
    alibi_to_blocks, bit_to_binary_mask, downcast_many, dummy_x, reshare_3_to_2, reveal_to_party,
    set_low_u32,
    types::{BitShare, input},
    upcast_x_to_block_many, y_from_block_pairs, y_to_block_pairs,
};
...
impl OhTable {
    pub fn new<N: Network>(
        ...
        let mut table = Self {
            ...
            stash_ys: vec![YRecord::zero_share(); params.stash_size],
            ...
            ys_builder_order: vec![YRecord::zero_share(); params.total_size()],
            dummy_indices: vec![XShare::zero_share(); params.num_dummies],
            xs_receiver_order: vec![XShare::zero_share(); params.total_size()],
            ys_receiver_order: vec![YRecord::zero_share(); params.total_size()],
            cht_2shares: None,
    ...
    pub fn extract(&self, extract_xs: &mut Vec<XShare>, extract_ys: &mut Vec<YRecord>) {
    ...
    fn shuffle_builder_order<N: Network>(
        ...
        let values = YRecord::get_y_values(&self.ys_builder_order);
        let alibis = YRecord::get_alibis(&self.ys_builder_order);
        let mut xs_blocks = upcast_x_to_block_many(&self.xs_builder_order);
        let (mut ys_low, mut ys_high) = y_to_block_pairs(&values);
        ...
        ArrayShuffler::new(self.params.total_size(), state).shuffle_many(
            &mut [
                ...
                ys_low.as_mut_slice(),
                ys_high.as_mut_slice(),
                alibi_blocks.as_mut_slice(),
            ],
        ...
        self.ys_builder_order = YRecord::from_columns(
            y_from_block_pairs(ys_low, ys_high),
    ...
    fn shuffle_receiver_order<N: Network>(
        ...
        let values = YRecord::get_y_values(&self.ys_receiver_order);
        let alibis = YRecord::get_alibis(&self.ys_receiver_order);
        ...
        self.ys_receiver_order = YRecord::from_columns(
            y_from_block_pairs(ys_low, ys_high),
            alibi_from_blocks(alibi_blocks),
        );
